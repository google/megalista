...
def _get_executions():
    return ExecutionsGroupedBySource(
        ...
        [
            Execution(
                None,
                Source('source_name', SourceType.BIG_QUERY, []),
                Destination(
                    'destination_name',
...
def bq_data_source_uuid():
    return BigQueryDataSource(
        _get_executions(), TransactionalType.UUID, bq_ops_dataset, bq_location
    )
...
def bq_data_source_gclid_time():
    return BigQueryDataSource(
        _get_executions(), TransactionalType.GCLID_TIME, bq_ops_dataset, bq_location
    )


@pytest.fixture
def bq_data_source_order_id_time():
    return BigQueryDataSource(
        _get_executions(), TransactionalType.ORDER_ID_TIME, bq_ops_dataset, bq_location
    )
...
def test_get_table_name(
    mocker,
    bq_data_source_uuid,
    bq_data_source_gclid_time,
    bq_data_source_non_transactional,
):
    ...
    assert (
        bq_data_source_uuid._get_table_name(source_metadata, True)
        == result_transactional_uploaded
    )
    assert (
        bq_data_source_gclid_time._get_table_name(source_metadata, True)
        == result_transactional_uploaded
    )
    assert (
        bq_data_source_uuid._get_table_name(source_metadata, False)
        == result_transactional_non_uploaded
    )
    assert (
        bq_data_source_gclid_time._get_table_name(source_metadata, False)
        == result_transactional_non_uploaded
    )
    assert (
        bq_data_source_non_transactional._get_table_name(source_metadata, False)
        == result_non_transactional
    )


def test_get_schema_fields(mocker, bq_data_source_uuid, bq_data_source_gclid_time):
    assert bq_data_source_uuid._get_schema_fields() == (
        SchemaField("uuid", "string"),
        SchemaField("timestamp", "timestamp"),
    )
    assert bq_data_source_gclid_time._get_schema_fields() == (
...
def test_bigquery_write_transactional_uuid(mocker, bq_data_source_uuid):
    ...
    bq_data_source_uuid.write_transactional_info(
        [{"uuid": "uuid-1"}, {"uuid": "uuid-2"}], execution
    )

    bq_client.insert_rows.assert_called_once_with(
        table,
        [{"uuid": "uuid-1", "timestamp": now}, {"uuid": "uuid-2", "timestamp": now}],
        (SchemaField("uuid", "string"), SchemaField("timestamp", "timestamp")),
    )


def test_bigquery_write_transactional_order_id_time(
    mocker, bq_data_source_order_id_time
):
    bq_client = mocker.MagicMock()
    now = datetime.datetime.now().timestamp()

    mocker.patch.object(bq_data_source_order_id_time, "_get_bq_client")
    bq_data_source_order_id_time._get_bq_client.return_value = bq_client
    mocker.patch.object(bq_data_source_order_id_time, "_get_now")
    bq_data_source_order_id_time._get_now.return_value = now
    ...
    bq_data_source_order_id_time.write_transactional_info(
        [
            {"order_id": "order_id1", "time": "ahorita"},
            {"order_id": "order_id2", "time": "ahorita_mas_uno"},
        ],
        execution,
    )
    ...
    bq_client.insert_rows.assert_called_once_with(
        table,
        [
            {"order_id": "order_id1", "time": "ahorita", "timestamp": now},
            {"order_id": "order_id2", "time": "ahorita_mas_uno", "timestamp": now},
        ],
        (
...
def test_bigquery_write_transactional_gclid_time(mocker, bq_data_source_gclid_time):
    ...
    account_config = AccountConfig("account_id", False, "ga_account_id", "", "")
    destination = Destination(
        "dest1", DestinationType.ADS_OFFLINE_CONVERSION, ["conversion"]
    )
    source = Source("orig1", SourceType.BIG_QUERY, ["dt1", "buyers"])
    execution = Execution(account_config, source, destination)
    ...
    bq_data_source_gclid_time.write_transactional_info(
        [
            ...
            {"gclid": "gclid2", "time": "ahorita_mas_uno"},
        ],
        execution,
    )
