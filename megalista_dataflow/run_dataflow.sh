...
if [[ -z "${CONFIG_PATH}" ]]; then
    ...
    exit
elif [[ -n "${2}" && "${2}" != "enable_prime" ]]; then
    show_message "${bold}${text_red}ERROR: Leave blank to run the default Dataflow job or type in enable_prime to run a Dataflow Prime job"
    exit
else
   init $CONFIG_PATH $2
fi