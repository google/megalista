...
class SpreadsheetExecutionSource(BaseBoundedSource):
    ...
    def __init__(self, sheets_config: SheetsConfig, setup_sheet_id: ValueProvider):
        super().__init__()
    ...
    def read(self, range_tracker):
        ...
        if self._sheets_config.check_if_range_exists(
            sheet_id, "CampaignManagerProfileId"
        ):
            campaign_manager_profile_id = self._sheets_config.get_value(
                sheet_id, "CampaignManagerProfileId"
            )
        else:
            campaign_manager_profile_id = self._sheets_config.get_value(
        ...
        if "values" in schedules_range:
            for schedule in schedules_range["values"]:
                if schedule[0] == "YES":
                    ...
                    yield Execution(
                        account_config, sources[schedule[1]], destinations[schedule[2]]
                    )
        else:
