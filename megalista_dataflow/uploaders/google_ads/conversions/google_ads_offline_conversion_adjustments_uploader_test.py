...
def uploader(mocker, error_notifier):
  ...
  return GoogleAdsOfflineAdjustmentUploaderDoFn(credentials,
...
def test_conversion_upload_with_gclids(mocker, uploader):
  ...
  uploader._get_oca_service.return_value.upload_conversion_adjustments.return_value = upload_return_mock
  ...
  uploader._get_oca_service.return_value.upload_conversion_adjustments.assert_called_once_with(request={
    'customer_id': '12345567890',
...
def test_upload_with_ads_account_override(mocker, uploader):
  ...
  conversion_name = 'user_list'
  destination = Destination(
    'dest1', DestinationType.ADS_OFFLINE_CONVERSION_ADJUSTMENT, ['user_list','987-7654-123'])
  source = Source('orig1', SourceType.BIG_QUERY, ['dt1', 'buyers'])
  execution = Execution(_account_config, source, destination)
...
def test_error_notification(mocker, uploader, error_notifier):
  ...
  assert error_notifier.destination_type is DestinationType.ADS_OFFLINE_CONVERSION_ADJUSTMENT
  assert error_notifier.errors_sent == {execution: f'Error on uploading offline conversion adjustments: {error_message}.'}
