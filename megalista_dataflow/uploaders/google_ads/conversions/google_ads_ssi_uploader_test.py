...
from uploaders.google_ads.conversions.google_ads_ssi_uploader import GoogleAdsSSIUploaderDoFn
...
def uploader(mocker):
    ...
    return GoogleAdsSSIUploaderDoFn(credentials,
...
@pytest.fixture
def ssi_batch_with_account_and_currency_override():
...
def ssi_batch_with_consent_override():
    return create_batch(_account_config, '', '', 'GRANTED', 'GRANTED')

def create_batch(account_config, currency_override, account_override, user_data_consent, ad_personalization):
    source = Source('orig1', SourceType.BIG_QUERY, ('dt1', 'buyers'))
    destination = Destination('dest1', DestinationType.ADS_SSI_UPLOAD,
                              [_conversion_name, _external_upload_id, _should_hash, currency_override, account_override, user_data_consent, ad_personalization])
...
def test_conversion_upload_account_and_currency_override(mocker, uploader, ssi_batch_with_account_and_currency_override):
    mocker.patch.object(uploader, '_get_offline_user_data_job_service')
    mocker.patch.object(uploader, '_get_resource_name')
    ...
    uploader.process(ssi_batch_with_account_and_currency_override)
...
def test_conversion_mcc_account_override(mocker, uploader, ssi_batch_with_mcc_account_override):
    mocker.patch.object(uploader, '_get_offline_user_data_job_service')
    mocker.patch.object(uploader, '_get_resource_name')
...
def test_conversion_upload_with_consent(mocker, uploader, ssi_batch_with_consent_override):
    mocker.patch.object(uploader, '_get_offline_user_data_job_service')
    mocker.patch.object(uploader, '_get_resource_name')
